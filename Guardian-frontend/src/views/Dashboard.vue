    ...
    methods:{
      getFileNum() {
        ...
        this.request.get('/ipfs/getList', {
          ...
          this.fileNum = this.data.length

          console.log('Total number of items:', this.fileNum); // 输出总条目数
        });
      },
      getDownloadNum() {
        let userId = this.user.id;
        this.request.get('/ipfs/getDownloadList', {
          ...
          this.downloadNum = this.data.length; // 使用文件数目更新 downloadNum 的值

          console.log('Total number of download items:', this.downloadNum); // 输出总条目数
        }).catch(err => {
